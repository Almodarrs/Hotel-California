...
@page { margin: 0; }
    ...
    $re = $pdo->query($sql);
   // $re->execute(['id'=>$pid]);
	while($row= $re->fetch(PDO::FETCH_BOTH))
	{
		$id = $row['id'];
	...
	?>
		<header>
			...
			<address >
				...
				<p>Albert Cuypstraat,<br>Amsterdam,<br>Netherlands.</p>
				<p>+31 (6)875-69-42</p>
			</address>
			<span><img alt="" src="public/cal.png"></span>
		</header>
		<article>
			<h1>Recipient</h1>
			<address >
		...
		<aside>
			...
			<div >
				<p align="center">Email :- info@hotel-ca.com || Web :- www.hotel-ca.com || Phone :- +31 (6)875-69-42 </p>
			</div>
		</aside>
	</body>
</html>
...
$free="Free";
$nul = null;
$rpsql = "UPDATE `rooms` SET `place`='$free',`category_id`='$nul' where `category_id`='$id'";
$rpsql_stmt = $pdo->prepare($rpsql);
$rpsql_stmt->execute(['place'=>$free,'category_id'=>$nul,'category_id'=>$id]);
if($rpsql_stmt->fetch(PDO::FETCH_OBJ))
{
	$delsql= "DELETE FROM `roombook` WHERE id='$id' ";
    $delsql_stmt = $pdo->prepare($delsql);
    $delsql_stmt->execute(['id'=>$id]);
	if($delsql_stmt->fetch(PDO::FETCH_OBJ))
	{
